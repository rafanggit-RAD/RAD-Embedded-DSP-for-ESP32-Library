...
bool I2S::begin(i2s_port_t port, int sampleRate, int bitsPerSample, bool isMaster, int bckPin, int wsPin, int dataOutPin, int dataInPin, int mclkPin, bool useInternalDAC, int dmaBufCount, int dmaBufLen) {
    ...
    if (!useInternalDAC) {
        // Set .mck_io_num ke -1 (I2S_PIN_NO_CHANGE) untuk mencegah driver internal double-registering clock
        // yang memicu error 'Selected io is already mapped by another signal' di Core 3.0.
        // Perutean MCLK fisik akan ditangani sepenuhnya secara manual oleh register IO MUX di bawah.
